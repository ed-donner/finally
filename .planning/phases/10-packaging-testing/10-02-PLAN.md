...
test.describe('Watchlist CRUD', () => {
  ...
  test('remove a ticker from watchlist', async ({ page }) => {
    ...
    // The remove button is an "x" that appears on hover of a PriceCell row
    // We need to hover over the NFLX row to reveal it
    const nflxRow = page.getByText('NFLX', { exact: true }).first().locator('..');
    await nflxRow.hover();

    // Click the "x" remove button within that row's parent container
    // The PriceCell has a button with text "x" that appears on group-hover
    const removeButton = nflxRow.locator('..').locator('button', { hasText: 'x' });
...
test.describe('Trading', () => {
  ...
  test('sell shares increases cash and updates position', async ({ page }) => {
    ...
    // The position should still exist but with quantity 5
    // Check that the qty column shows 5 for AAPL
    await expect(positionsTable.getByText('5')).toBeVisible({ timeout: 10000 });
  });
